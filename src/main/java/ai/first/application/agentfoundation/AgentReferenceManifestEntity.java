...
public class AgentReferenceManifestEntity extends EventSourcedEntity<AgentReferenceManifestEntity.State, AgentReferenceManifestEntity.Event> {
  ...
  public Effect<AgentReferenceManifest> save(AgentReferenceManifest manifest) {
    ...
    return effects().persist(new Event.ReferenceManifestSaved(manifest, lifecycleFact(manifest))).thenReply(State::manifest);
  }

  @Override
  public State applyEvent(Event event) {
    return switch (event) {
      case Event.ReferenceManifestSaved saved -> currentState().save(saved.manifest(), saved.lifecycleFact());
  ...
  private Optional<String> validateLifecycleFact(GovernedArtifactLifecycleFact fact) {
    ...
    if (fact.artifactType() != GovernedArtifactLifecycleFact.ArtifactType.AGENT_REFERENCE_MANIFEST) return Optional.of("artifact-type-mismatch");
  ...
  private GovernedArtifactLifecycleFact lifecycleFact(AgentReferenceManifest manifest) {
    var previous = currentState().manifest();
    return GovernedArtifactLifecycleFact.of(
        manifest.tenantId(),
        GovernedArtifactLifecycleFact.ArtifactType.AGENT_REFERENCE_MANIFEST,
        manifest.manifestId(),
  ...
  private GovernedArtifactLifecycleFact noOpFact(AgentReferenceManifest manifest) {
    return GovernedArtifactLifecycleFact.of(
        manifest.tenantId(),
        GovernedArtifactLifecycleFact.ArtifactType.AGENT_REFERENCE_MANIFEST,
        manifest.manifestId(),
  ...
  public record State(AgentReferenceManifest manifest, List<GovernedArtifactLifecycleFact> history) {
    static State empty() {
    ...
    State save(AgentReferenceManifest manifest, GovernedArtifactLifecycleFact lifecycleFact) {
  ...
  public sealed interface Event {
    ...
    record ReferenceManifestSaved(AgentReferenceManifest manifest, GovernedArtifactLifecycleFact lifecycleFact) implements Event {}
    @TypeName("agent-reference-manifest-lifecycle-fact-appended")
    record LifecycleFactAppended(GovernedArtifactLifecycleFact lifecycleFact) implements Event {
      @Override public AgentReferenceManifest manifest() { return null; }
    }
  }
