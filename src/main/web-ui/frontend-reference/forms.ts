...
export function readAndValidate(elements: Elements): ValidationResult {
...
export function renderFieldErrors(elements: Elements, errors: FieldError[]): void {
  ...
  for (const error of errors) {
    ...
    const target = elements[`${error.field}Error` as const];
