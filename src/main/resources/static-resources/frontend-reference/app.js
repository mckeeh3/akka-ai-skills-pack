...
window.addEventListener("DOMContentLoaded", () => {
    ...
    let state = { ...initialState(), dashboard: { status: "loading" } };
    const rerender = () => render(elements, state);
    ...
    elements.form.addEventListener("submit", async (event) => {
        ...
        if (result.ok) {
            elements.form.reset();
            state = withSubmittedRequest({ ...state, submit: { status: "success", message: result.value.message } }, result.value.request);
        }
        else {
    ...
    void (async () => {
        ...
        state = result.ok
            ? {
                ...state,
                dashboard: result.value.requests.length === 0
                    ? { status: "empty" }
