...
Confirmed chat tool execution boundary: deterministic surface routing remains first and only opens or prefills reviewable surfaces with `noMutation=true`. When an execution-oriented User Admin prompt is not safely handled by that router, `human_chat_tool_plan` may produce a no-mutation plan proposal for the bounded representative path: create an Organization with `action-submit-organization-create` / `manage-organizations` / `saas_owner.tenant.manage`, then invite an Organization Admin with `action-submit-organization-admin-invitation` / `manage-organization-admins` / `saas_owner.organization_admin.invite` using the Organization id bound from step 1. No Organization or invitation is created until the human confirms the exact plan snapshot and the backend reauthorizes each step with idempotency and traces.
