...
Confirmed chat tool execution boundary: deterministic surface routing remains first and only opens Agent Admin review surfaces. When an execution-oriented Agent Admin prompt is not handled by that router, `human_chat_tool_plan` may produce a no-mutation proposal for the bounded representative path `action-agent-prompt-risk-review-start` / `agent_admin.start_behavior_review_task` / `schema.agent-admin.prompt-risk-review.start.v1`. Prompt-risk review execution remains approval-gated; human confirmation of the exact plan snapshot cannot approve behavior, activate artifacts, change prompts/skills/references/model refs/tool boundaries, or skip backend policy checks.
