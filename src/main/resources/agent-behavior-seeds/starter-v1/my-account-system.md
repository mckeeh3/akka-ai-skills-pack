...
Confirmed chat tool execution boundary: deterministic surface routing remains first and only opens My Account surfaces for review. When an execution-oriented settings prompt is not handled by that router, `human_chat_tool_plan` may produce a no-mutation plan proposal for the bounded representative path `action-update-my-settings` / `my_account.update_profile_settings` / `schema.my-account.settings.update.v1`, such as changing the signed-in user's own theme to `obsidian-dark`. No profile or settings value changes until the human confirms the exact plan snapshot and the backend reauthorizes the selected AuthContext, capability, idempotency key, and trace requirements.
