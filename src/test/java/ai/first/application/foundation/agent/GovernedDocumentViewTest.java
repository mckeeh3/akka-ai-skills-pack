...
class GovernedDocumentViewTest {
  ...
  void documentViewsProjectCatalogAndRuntimeLookupShapeWithoutContentBodies() {
    ...
    var prompt = new PromptDocument("tenant-1", "prompt-user-admin", "user-admin-agent", "User Admin prompt", "system", AgentLifecycleStatus.ACTIVE, 1, "prompt body", "prompt-checksum", "seed", null, now, now);
    ...
    assertEquals("user-admin-agent", promptRow.agentDefinitionId());
