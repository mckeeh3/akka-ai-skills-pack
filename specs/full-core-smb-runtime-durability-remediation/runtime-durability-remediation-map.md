...
| Invitation repository has durable Akka seam but normal static registry still defaults to in-memory. | `InMemoryInvitationRepository.java`, `AkkaInvitationRepository.java`, `DurableInvitationRepositoryEntity.java`, `InvitationService.java`, `StarterSecurityComponents.java` | historical normal-runtime default requiring rebinding | blocker | Bind normal invitation runtime to `AkkaInvitationRepository`/`DurableInvitationRepositoryEntity`; keep substitutes test-only. Validate idempotent invite/resend/revoke/outbox paths. |
