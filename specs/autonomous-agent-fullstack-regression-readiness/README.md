...
The Governance/Policy Impact validation recorded that targeted checks pass, but full scaffolded `mvn test` is blocked by a stale/unrelated Audit/Trace test expectation: `WorkstreamServiceTest.auditTraceSummaryWorkerFailsClosedUntilRealAutonomousRuntimeExists` expects `audit.trace.summaryTask.v1` while current source returns `audit.trace.summaryProgress.v1`.
