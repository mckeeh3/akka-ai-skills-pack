# Add model-governance tests

## Objective

Prove the starter model binding path with tests.
...
Add backend tests for active model success, disabled/unknown or unauthorized model denial, provider-policy denial, no secret exposure, fallback/no-fallback behavior where implemented, and model refs in traces.

## Non-goals

Do not rely on docs-only proof.
