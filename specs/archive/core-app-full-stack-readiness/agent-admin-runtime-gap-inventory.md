...
| Frontend | Workstream fixtures include `Agent Admin` rail item with denied state. Starter core app-description has Agent Admin UI references. | Gap: no concrete Agent Admin workstream components/fixtures for definitions, prompts, skills, manifests, tool boundaries, model refs, test console, proposals, or traces. |
