...
| Active specs previously cited legacy app-description paths | `specs/full-core-saas-readiness/pending-tasks.md` and task briefs previously cited paths like `app-description/12-workstreams/surface-contracts/...` | Reconciled by `TASK-ADICM-03-001` to current graph nodes under `app-description/global/**` and `app-description/domains/core-starter/**`; remaining archive/provenance references are scrubbed by Sprint 04. |
