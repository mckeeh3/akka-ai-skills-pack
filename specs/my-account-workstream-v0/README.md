...
The five-core v0 starter already exists as a production-ready baseline. This workstream project plans the next focused vertical increment for the `My Account Agent` without re-planning the other four workstreams.

## Agent-type intent

My Account is primarily request/response plus deterministic account/context services. AutonomousAgent use is optional and should appear only for durable personal audit digest or long-running self-service review work that has real task lifecycle semantics.
