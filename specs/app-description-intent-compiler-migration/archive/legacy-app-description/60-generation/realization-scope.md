...
- phase-1 secure SaaS foundation:
  ...
  - admin UI/API surfaces for Users, Invitations, Roles/Memberships, Access Review, Support Access, Admin Audit, Tenant/Customer Settings, admin-agent recommendations, agent catalog/detail, prompt/skill governance, skill manifests, tool permissions, edit-agent proposal review, managed-agent trace exploration, and the provider-neutral enterprise audit/export request lifecycle foundation for digest, export, legal-hold marker, e-discovery, SIEM fail-closed/local export, and compliance-report handles
...
- phase-4 integration expansions:
  - consumers, timed actions, realtime updates, digest generation, export artifact/provider delivery extensions beyond the current redacted local enterprise audit/export request handles
