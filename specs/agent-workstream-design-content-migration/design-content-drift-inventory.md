...
| DCA example UI shape | `docs/examples/ai-first-dca-app-description/app-description/55-ui/README.md`, `docs/examples/ai-first-dca-app-description/app-description/55-ui/ui-surfaces.md`, many DCA capability files linking to `../55-ui/ui-surfaces.md` | DCA keeps a consolidated `ui-surfaces.md` and lacks the starter core app's split `12-workstreams/` layer and split `55-ui/` file set. The README says future downstream realization may split it, but it still risks looking canonical. | align now / label as legacy or reference-specific | `TASK-AWDD-04-001`: either migrate DCA to the canonical split or label the consolidated UI as historical/reference-specific mechanics. |
...
| Seed example as canonical target | `templates/ai-first-saas-starter/app-description/**` | Starter core app contains `12-workstreams/` and a rich split `55-ui/` layer. It is the best current target model, but the exact file set differs from bootstrap/UI skill guidance (`ai-first-surfaces.md` exists in seed/UI skill but not internal architecture's default list). | leave unchanged now; use as input | `TASK-AWDD-02-002`: use seed plus doctrine to define the final canonical file set. |
...
|---|---|---|---|
| `templates/ai-first-saas-starter/app-description/**` | Best current target | Has `12-workstreams/` and rich split `55-ui/`. It should be treated as the canonical secure AI-first SaaS starter core reference. Minor drift: exact `55-ui/` files are richer than internal architecture and differ from bootstrap. | leave unchanged now; use in target definition |
