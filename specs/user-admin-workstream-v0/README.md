...
Complete the User Admin vertical for invitations, memberships, roles/capabilities, access review, and admin-safe next steps.
...
The five-core v0 starter already exists as a production-ready baseline. This workstream project plans the next focused vertical increment for the `User Admin Agent` without re-planning the other four workstreams.

## Agent-type intent

User Admin should deliberately demonstrate all three agent categories: request/response User Admin Agent turns, AutonomousAgent-backed access-review/admin-risk investigations, and deterministic policy/invitation/membership lifecycle services.
