...
| `user-admin-user-list` | `user-admin-agent` | dashboard queues and prompt/deep-link requests | user search/list/invite draft/open detail, including Organization Admin list filtered from SaaS Owner Organization detail → `secure-tenant-user-foundation`; audit excerpts → `governance-decisions-audit`. |
