...
function validate_task(title, body, status,    text, exempt) {
  ...
  text = tolower(body)
  exempt = (text ~ /(internal-only|foundation-only|cross-cutting|docs-only|non-runtime)/)
  missing = ""

  if (text !~ /(vertical workstream contract|vertical contract:)/) add_missing("vertical contract block/line")

  if (exempt) {
    if (text !~ /(internal-only|foundation-only|cross-cutting|docs-only|non-runtime)/) add_missing("explicit exemption scope")
    if (text !~ /(non-attention|no attention|non-ui|no ui|non-runtime|docs-only|internal-only|foundation-only|cross-cutting)/) add_missing("non-attention/non-UI reason")
    if (text !~ /(capability|foundation scope|scope)/) add_missing("capability or foundation scope")
    if (text !~ /(audit|trace|work trace|no trace|none)/) add_missing("audit/work trace expectation")
    ...
    if (text !~ /(governed-tool|governedtool|browser-tool|agent-tool|internal-tool|workflow-tool|timer-tool|consumer-tool|mcp-tool|api exposure)/) add_missing("governed-tool id and exposure")
