...
class DraftCartValidatorTest {

  @Test
  void validateAddItemCommand() {
    var errors =
        DraftCartValidator.validate(
            DraftCart.State.empty("cart-1"),
            new DraftCart.Command.AddItem("", "", 0));
  ...
  void validateRemoveItemCommand() {
    var errors =
        DraftCartValidator.validate(
            DraftCart.State.empty("cart-1"),
            new DraftCart.Command.RemoveItem(""));
  ...
  void validateCheckoutCommand() {
    var errors =
        DraftCartValidator.validate(
            DraftCart.State.empty("cart-1"),
            new DraftCart.Command.Checkout());
