...
public class AuditTraceSummaryResultRule implements TaskRule<AuditTraceSummaryResult> {
  @Override
  public Result onComplete(AuditTraceSummaryResult result) {
    ...
    if (result.findings().stream().anyMatch(finding -> finding.evidenceRefs().isEmpty())) {
      return new Result.Rejected("each finding must cite redacted evidence refs");
    }
    var text = result.toString();
    if (containsSecretOrRawPayload(text)) {
      return new Result.Rejected("result must not expose provider secrets, raw prompts, JWTs, tokens, credentials, or raw tool payloads");
    }
    if (claimsProtectedMutation(text)) {
      return new Result.Rejected("Audit/Trace summary result must not claim protected mutations");
    }
  ...
  private static boolean containsSecretOrRawPayload(String value) {
    ...
    return lower.contains("api_key")
        ...
        || lower.contains("providercredential")
        || lower.contains("raw prompt")
        || lower.contains("raw tool payload");
  }

  private static boolean claimsProtectedMutation(String value) {
    var lower = value == null ? "" : value.toLowerCase(Locale.ROOT);
    return lower.contains("mutated roles")
        || lower.contains("changed policy")
        || lower.contains("updated provider config")
        || lower.contains("deleted trace")
        || lower.contains("disabled user");
  }
}
