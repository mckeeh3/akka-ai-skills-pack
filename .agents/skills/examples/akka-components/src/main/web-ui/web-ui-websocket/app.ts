function appendLog(target: HTMLUListElement, message: string): void {
  const item = document.createElement("li");
  item.textContent = message;
  target.appendChild(item);
}

function toSocketUrl(socketPath: string): string {
  const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
  return `${protocol}//${window.location.host}${socketPath}`;
}

function startSocketDemo(): void {
  const shell = document.querySelector<HTMLElement>("[data-socket-path]");
