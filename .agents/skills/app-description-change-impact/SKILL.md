...
description: Determine which app-description current-intent graph nodes, traceability artifacts, readiness state, implementation areas, and generated/derived outputs are affected by a requested description change, and decide whether realization can stay localized or should broaden.
---
...
- target project path: app-description/global/**
- target project path: app-description/domains/<domain>/capabilities/**
- target project path: app-description/domains/<domain>/workstreams/<workstream>/**
- target project path: app-description/domains/<domain>/workstreams/<workstream>/realization/**
...
- "which graph nodes must be updated because of this behavior change?"
...
- which workstream agent/expertise bindings are impacted: bundle scope, prompt intent, skill/reference ids, compact manifest entries, capability map, `ToolPermissionBoundary`, authority profile, loader denials, governed-tool explanations, trace obligations, governance owner, default-content upgrade behavior, or tests
- which operating-model graph nodes are impacted for generated AI-first SaaS: goals, delegated work, retained human authority, agent/team responsibilities, policies, approval gates, decisions, exceptions, evidence, traces, learning, or outcomes
...
- to `app-description-ui` when workstream shell, rail/composer, structured surface rendering, supervision, decision, governance, digest, goal-to-execution, or audit UI implications exist
...
- to `app-generate-app` only after the impacted current-intent graph nodes are updated and readiness is acceptable
...
- impacted authoritative graph nodes are named explicitly, including domain capabilities, workstream agent/expertise bindings, policy/tool/surface/trace bindings, and realization maps for generated AI-first SaaS apps
- impacted derived artifacts are named explicitly
