...
Each surface action is a browser-tool or surface-request exposure of a governed capability. Use `governed-tool` for the semantic executable contract and qualified exposure terms (`browser-tool`, `human_chat_tool_plan`, `agent-tool`, `workflow-tool`, `timer-tool`, `consumer-tool`, `MCP-tool`, `internal-tool`) when describing where it is exposed. A surface action and a confirmed chat plan may expose the same governed-tool, but the surface contract remains responsible only for the browser/surface adapter; the chat-plan adapter belongs in the workstream tool map and expertise/runtime contract.
...
- selected capability exposure channels: structured surface action, confirmed human chat tool plan, browser API, agent tool, workflow step, timer, consumer, MCP, view, or internal method;
