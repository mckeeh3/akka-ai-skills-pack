...
Use this pattern for generated secure AI-first SaaS workstreams that need durable internal/background model-driven work with a real Akka `AutonomousAgent` task lifecycle. It is extracted from the starter User Admin access-review worker and Agent Admin prompt-risk review worker.
...
Use Akka `AutonomousAgent` when the work is internal/background, task-oriented, and benefits from typed task ids, snapshots, failure/cancellation, notifications, dependencies, or model-driven iteration. Common generated-app examples include access review investigations, prompt/skill/reference risk review, audit summaries, evaluation/replay loops, monitoring/remediation, and specialist follow-up.
...
- task-specific flags such as `activationBlockedUntilHumanDecision=true` where protected activation is out of scope;
