...
| Email-invite onboarding | Complete onboarding through mandatory `Invitation` lifecycle: invite token or acceptance context, status, expiry, resend, revoke/cancel, acceptance, delivery status, delivery attempts, idempotency, and audit trail. Production readiness defaults to Resend (resend.com) email delivery configuration; local/dev/test must use an explicit captured outbox adapter. Use another production provider only when the project explicitly records an accepted override decision. |
