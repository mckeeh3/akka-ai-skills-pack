...
Applications generated by this skills pack are **secure AI-first SaaS applications by default**. Use this target from the start of every new project, PRD, app-description, backlog, or generation request unless the user explicitly asks for repository-maintenance-only or non-SaaS reference material.

Security is mandatory from the first planning step, never a later hardening phase, and never dependent on whether the user mentions it. Every generated SaaS design starts with a mandatory secure SaaS foundation and then adds app-specific AI-first/domain behavior.
