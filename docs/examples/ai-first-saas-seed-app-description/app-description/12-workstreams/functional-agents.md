...
| `user-admin-agent` | Invitations, users, memberships, roles/capabilities, access review, support-access visibility, and admin audit. It owns the User Admin role-specific dashboard trunk, human surface graph, and internal worker delegation graph. Authoritative expertise: `workstream-expertise/user-admin-agent.md`. | Tenant Admin, Customer Admin within scope, SaaS Owner support roles with explicit support grant. | `user-admin-dashboard` with `user-admin-user-list`, `user-admin-user-account`, `decision-card`, and `audit-trace-explorer` as canonical graph nodes | `secure-tenant-user-foundation`, `managed-agent-foundation`, `governance-decisions-audit` | invite lifecycle including expired-invitation resend/revoke decisions, tenant isolation, last-admin protection, role denial, skill/reference load denial, AdminAuditEvent and AgentWorkTrace coverage. |
...
### Role dashboard and graph contract

`user-admin-dashboard` is the trunk of this workstream. Its attention model includes pending, failed, stale, and expired invitations; dormant admins; support-access expiry; last-admin risk; access-review items; internal-agent recommendations; and recent denied admin actions. Surface graph branches open scoped list/detail/evidence nodes or decision cards. Internal worker agents may prepare drafts, summaries, risk classifications, and recommendations, but every browser-tool, agent-tool, or internal-tool invocation still maps to governed capabilities and records traces.
