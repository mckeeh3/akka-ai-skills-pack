...
| `30-tests/` | Has acceptance, regression, negative, and operational directories with README placeholders. | Detailed executable-style test specs are intentionally deferred while DCA remains reference-only. | Missing concrete tenant-isolation, forbidden-access, disabled-user, role/scope denial, approval-bypass, idempotency, audit/trace, frontend secret-boundary, integration, and outcome verification specs. | Sprint 4 test refresh tasks. |
...
4. Tests are placeholders rather than acceptance/negative/regression/operational specifications.
5. UI and observability layers are compact vertical summaries, not generation-ready contracts.
