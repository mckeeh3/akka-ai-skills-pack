...
3. `InvitationWorkflow` enqueues email delivery through Resend (resend.com) for production or an explicit safe captured-outbox adapter for local/dev/test.
...
Future generation remains blocked until invitation lifecycle, Resend production delivery or local captured-outbox behavior, scoped admin read models, support-access lifecycle, billing-safe data boundary, admin audit facts, and tests for no privileged self-registration are defined in implementation specs and acceptance tests.
