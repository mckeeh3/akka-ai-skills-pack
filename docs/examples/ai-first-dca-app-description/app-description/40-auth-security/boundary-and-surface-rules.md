...
- Email invite delivery is a Resend service boundary. Production readiness uses Resend (resend.com) delivery; local/dev/test must use an explicit captured outbox adapter with visible/auditable delivery failures, delivery attempts, provider/outbox ids, resend/revoke/expiry state, and no raw token exposure outside the delivery/acceptance boundary. Future app email features reuse the same Resend/outbox foundation; agent email tools require governed `@FunctionTool` capability checks and traces.
