...
Canonical full-core implementation reference in this source repository and downstream forks: the runnable core app repository root. Reusable workstream UI architecture and source-reference modules live in `docs/workstream-ui-reference-architecture.md`, root `frontend/src/workstream/**`, and root `frontend/src/workstream-user-admin-vertical.contract.test.mjs`. The skills-only install does not export `resources/examples/frontend/**`; application code belongs in the target project. The User Admin vertical is the canonical foundation-admin UI pattern for dashboard → list/search → detail/edit behavior through structured surfaces. Endpoint-only and static-resource examples below are delivery mechanics references; they are not canonical generated SaaS app structure.
