...
Canonical full-core implementation reference in this source repository: the upstream runnable core app repository root. In an installed pack, use `resources/examples/frontend/**` only as reusable UI reference source; application code belongs in the target project. Reusable workstream UI architecture and source-reference modules live in `docs/workstream-ui-reference-architecture.md`, `frontend/src/workstream/**`, and `frontend/src/workstream-user-admin-vertical.contract.test.mjs`; installed packs export those UI references under `resources/examples/frontend/**`. The User Admin vertical is the canonical foundation-admin UI pattern for dashboard → list/search → detail/edit behavior through structured surfaces. Endpoint-only and static-resource examples below are delivery mechanics references; they are not canonical generated SaaS app structure.
