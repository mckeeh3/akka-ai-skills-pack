...
Every workstream surface action is a browser-tool exposure backed by a governed-tool inside a governed backend capability, including read/query actions and surface-request actions such as `show_surface`, `open_workstream`, show dashboard, search, open detail, refresh, or open trace. Frontend action availability is only a UX hint. Surface-request actions may carry `shellRequest` metadata so buttons, links, cards, rows, counters, badges, chart segments, task/progress panels, My Account panels, rail entries, and deep links use the same canonical prompt feedback and origin-aware request item rendering as composer-entered prompts. Dashboard objects representing attention or next work are clickable and keyboard-operable by default, append a request-like workstream item, and append/open the target/result surface; ready dashboard payloads should omit forbidden targets rather than rendering inert disabled work objects. The action descriptor must identify the source surface, target/result/system-message surface behavior, browser-tool name, governed-tool id, capability id, authorization basis, idempotency, and audit/trace requirements.
...
- surface envelopes for every canonical surface type listed above, including collection list/search, object show, create, edit, and destructive lifecycle confirmation surfaces when a collection object is in scope, aligned to current-intent workstream surface nodes (`app-description/domains/<domain>/workstreams/<workstream>/surfaces/**`) when implementing SaaS Foundation App surfaces; legacy/template `templates/ai-first-saas-core-app/app-description/12-workstreams/**` files may be used only as mapped compatibility references when they are still maintained
