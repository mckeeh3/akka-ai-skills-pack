...
4. **Dashboard things are operable.** Cards, rows, counters, badges, chart segments, task/progress panels, shortcuts, icons, and buttons that represent things needing attention or things the user can do next are clickable and keyboard-operable by default. Activation appends a request-like workstream item and appends/opens the surface where the user can inspect details and take allowed actions. Inert dashboard objects are explicit exceptions with a recorded reason.
