...
- **SaaS Foundation App role:** User Admin participates in the SaaS Foundation App domain; it is not a valid single-workstream SaaS Foundation App by itself, and the full User Admin scope requires the surfaces/capabilities below
...
|---|---|---|---|---|
| `surface.user_admin.dashboard.v1` | dashboard | action router for user-admin attention and available admin work: failed deliveries, last-admin risks, approval-required changes, stale access reviews, pending invites, disabled users, scoped user search, invite creation, access review, and audit drilldown | `user_admin.dashboard.view` | open failed-delivery queue, resolve last-admin risk, open approval queue, open access review, search users, open invite form, open invitations, open audit summary |
...
- Dashboard: render as an action-router mission control, not a report. The first region is clickable attention indicators for failed delivery, last-admin, approval-required, and stale access-review items; the second region is clickable next-action indicators for search users, invite user, open invitations, access review, and audit drilldown. Pending invite, disabled user, and admin-count tiles are allowed only when the whole tile opens the matching queue/detail/history surface, including useful `0` states; passive admin-health/recent-audit metrics belong in the audit/detail surfaces rather than as inert dashboard content.
...
A capability is the governed backend contract. It may be exposed through one or more actor adapters: human `surface_action`, confirmed `human_chat_tool_plan`, AI-backed `agent_tool_call`, browser/API `api_call`, internal-agent `internal_call`, `workflow_step`, `timer_invocation`, `consumer_reaction`, `mcp_tool_call`, view-backed evidence reads, or internal methods with provenance. Browser APIs, confirmed chat plans, and agent tools are exposure forms over the same capability; they do not redefine authorization, validation, idempotency, side effects, audit, approval, or denial behavior.
...
→ `surface_action` / browser `api_call`
...
- Agent: `UserAdminAgent` with read/proposal `agent_tool_call` adapters; side-effecting tools only through backend-authorized `surface_action`, modeled `human_chat_tool_plan` confirmation, approval flow, or explicitly bounded autonomous policy.
