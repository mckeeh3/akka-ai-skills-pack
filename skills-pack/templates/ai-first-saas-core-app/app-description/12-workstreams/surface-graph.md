...
| `saas-owner-organization-admin` | `user-admin-agent` | SaaS Owner Admin directory/detail/status/admin-bootstrap requests | SaaS Owner Organization/Tenant lifecycle workspace | `surface-contracts/08-saas-owner-organization-admin.md` |
...
| `user-admin.open-organization-admin` | `user-admin-dashboard` | open SaaS Owner Organization Administration | `saas-owner-organization-admin` | `secure-tenant-user-foundation` | Available only to SaaS Owner Admin capability; platform-safe metadata only. |
...
| `user-admin.manage-organization-lifecycle` | `saas-owner-organization-admin` | create/update/status-change Organization/Tenant, bootstrap Organization Admin, open admins/audit | `saas-owner-organization-admin`, `user-admin-user-list`, `decision-card`, `audit-trace-explorer`, or `system_message` | `secure-tenant-user-foundation` / `governance-decisions-audit` | SaaS Owner Admin only; no organization application data access. |
