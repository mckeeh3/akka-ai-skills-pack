...
Security and web UI baseline tasks must never be omitted as cross-cutting polish. For SaaS app queues, ensure the first runnable tasks implement or verify the full-stack secure foundation from `../docs/full-core-foundation-readiness.md` and, for minimum/basic/core app/chatbot-like generated SaaS, the five-core-workstream shell before domain-specific features. Split onboarding/email, admin search, governed-agent records/loaders/traces, AI admin offload, UI surfaces, audit, and security/frontend/agent-governance tests into bounded tasks. If the source backlogs lack those tasks or still describe a User-Admin-only core app, repair the queue only after adding or flagging the missing foundation backlog coverage instead of silently proceeding to domain work.
