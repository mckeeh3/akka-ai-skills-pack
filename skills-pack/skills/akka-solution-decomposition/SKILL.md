...
Context-budget rule: do not load every focused Akka component skill up front. First classify scope and component candidates, use `../docs/full-core-foundation-readiness.md` by reference for foundation scope, then load only the focused downstream skills needed for the selected substrates.
...
- `../docs/full-core-foundation-readiness.md` for the canonical full-core foundation checklist; do not restate that inventory inline
...
For section 3, label scope before choosing components using `../docs/full-core-foundation-readiness.md`. `core app baseline` is allowed for minimum/core app/basic/chatbot-like generated SaaS requests and must use `docs/minimum-ai-first-saas-app.md`, state that it is not full-core ready, and list full-core follow-up gates. `full core` must carry the canonical full-core scope through capability summary, component mapping, implementation order, and required tests. `Module 1-only / not full core` or any narrower scope must be named and must list deferred full-core areas.
...
That section must summarize the selected canonical foundation scope from `../docs/full-core-foundation-readiness.md` without restating the entire inventory inline. WorkOS/AuthKit is the supported browser authentication provider and Resend (resend.com) is the supported production email service; missing runtime setting values may become questions, but they must not erase local authorization, tenancy, managed-agent behavior, prompt/skill/reference governance, trace, or tool-boundary contracts. Route complete invitation onboarding work to `akka-saas-invitation-onboarding`; route reusable email service, future feature emails, and agent `@FunctionTool` email surfaces to `akka-resend-email-service`.
...
1. core secure SaaS foundation before app-specific domain features, using `../docs/full-core-foundation-readiness.md`; split identity/tenancy/auth, `/api/me`, authorization, onboarding/email, admin search, governed-agent records/loaders/traces, AI admin offload, workstream UI, audit, and security/frontend/agent-governance tests into vertical tasks
...
- `akka-saas-invitation-onboarding` for complete email-invite lifecycle, InvitationWorkflow, Resend email delivery/outbox Consumer, expiry/reminder TimedAction, InvitationView, resend, revoke, delivery status, acceptance, and invitation tests
- `akka-resend-email-service` for the shared Resend production email service, local captured outbox, future feature emails, and governed agent `@FunctionTool` email tools
- `akka-http-endpoints`
...
- `akka-agent-reference-governance` for `ReferenceDocument`/`ReferenceVersion`, `AgentReferenceManifest`, compact reference manifest context, authorized `readReferenceDoc(referenceId)`, `ReferenceLoadTrace`, and reference-governance UI
...
- Which policies, permissions, thresholds, prompts, skills, reference documents, or guardrails must be versioned and mechanically enforced?
