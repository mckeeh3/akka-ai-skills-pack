...
For minimum, core app, basic, basic-chatbot, smallest-useful-app, or initial chatbot-like generated SaaS requests, also read `../docs/minimum-ai-first-saas-app.md` before applying Slice 0 readiness.
...
- `akka-agent-behavior-profiles`, `akka-agent-governed-documents`, `akka-agent-prompt-governance`, `akka-agent-skill-governance`, `akka-agent-reference-governance`, and `akka-agent-work-trace` for the mandatory governed runtime agent foundation and first-install/tenant-bootstrap governed default behavior/reference setup
...
| Agent Admin | agent catalog/detail, prompt/skill/reference/version cards, manifest and tool-boundary editors, behavior-test console, proposal/diff review cards, trace links | manage `AgentDefinition`, governed prompt/skill/reference documents, manifests, tool boundaries, governed default setup/upgrade, behavior proposals, prompt assembly, authorized `readSkill(skillId)`, authorized `readReferenceDoc(referenceId)` |
...
Use `../docs/full-core-foundation-readiness.md` as the canonical full-core readiness checklist. This section summarizes the foundation concepts every generated SaaS application must model before app-specific features:
...
Minimum core app readiness must always carry follow-up tasks/gates for full-core SaaS readiness. It may defer complete WorkOS/AuthKit setup, invitation/onboarding, Resend/outbox, complete User Admin, Agent Admin governed documents, Audit/Trace UI, support access, billing boundary, and complete security coverage, but only as explicit follow-up work. It must not claim app-specific readiness.

### Full-core foundation readiness

Full-core readiness remains stricter than Slice 0. For full generated SaaS readiness, use the canonical checklist in `../docs/full-core-foundation-readiness.md`; the sequence below is retained as a route-local summary before app-specific domain-specific work:
...
- For core app baseline generation, the description/spec must satisfy five-core-workstream core app domain readiness and must emit follow-up tasks for full-core SaaS readiness.
...
- The target readiness state is explicit: core app baseline ready, full-core ready, or app-specific ready. Core app baseline ready means five-core-workstream core app domain with bootstrap auth, selected AuthContext, backend role/capability boundary, workstream logs, `markdown_response` surfaces, audit/work trace substrate, no public self-registration, and full-core follow-up tasks. Full-core ready and app-specific ready require the stricter checks below.
...
- SaaS Owner, Tenant, Customer, Account, UserProfile, UserSettings, Membership, Role, Permission/Capability, AuthContext, AdminAuditEvent, and the backend role/capability boundary are present for every generated SaaS readiness target, including the core app baseline. Complete Invitation lifecycle, first-slice UserDirectoryView, MembershipView, InvitationView, AdminAuditView, AccessReviewQueueView, support-access, subscription/billing boundary, and full foundation web UI surfaces are present for full-core/app-specific readiness; for core app baseline readiness, omissions are allowed only when explicitly recorded as full-core follow-up gates. Non-SaaS reference work must explicitly state which foundation concepts are intentionally out of scope.
...
- Governed runtime agent behavior routes through `akka-agent-behavior-profiles`, `akka-agent-governed-documents`, `akka-agent-prompt-governance`, `akka-agent-skill-governance`, `akka-agent-reference-governance`, and `akka-agent-work-trace`; missing `AgentDefinition`, `PromptDocument`/`PromptVersion`, `SkillDocument`/`SkillVersion`, `ReferenceDocument`/`ReferenceVersion`, `AgentSkillManifest`, `AgentReferenceManifest`, `ToolPermissionBoundary`, first-install governed setup for default behavior/reference documents, prompt assembly, authorized `readSkill(skillId)`, authorized `readReferenceDoc(referenceId)`, `PromptAssemblyTrace`, `SkillLoadTrace`, `ReferenceLoadTrace`, or `AgentWorkTrace` blocks generated AI-first SaaS foundation readiness.
