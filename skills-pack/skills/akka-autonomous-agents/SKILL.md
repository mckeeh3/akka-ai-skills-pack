...
Use this skill when implementing or reviewing an Akka Java SDK `AutonomousAgent`: a durable model-driven component that owns a task loop, typed tasks, snapshots/state, cancellation/failure, notifications, and optional model-driven coordination. In generated SaaS apps, every `AutonomousAgent` also has a governed managed `AgentDefinition` domain record that supplies runtime prompt, skill manifest, tool boundary, model config, and traces.
...
- caller `AuthContext`, tenant/customer scope, roles/capabilities, governed managed `AgentDefinition`, active prompt, skill manifest with mandatory `readSkill(skillId)`, model policy, tool boundary, approval gates, trace requirements, and provider-secret boundary;
...
- typed `worker.task.*` workstream events, attention upsert/resolve rules, structured progress/result surfaces, provider fail-closed behavior, and no fake success/runtime-substitute guardrails.
...
5. Keep static `.instructions(...)`, `.tools(...)`, `.mcpTools(...)`, `.modelProvider(...)`, guardrails, or content loaders minimal and subordinate to the governed runtime profile; generated-app runtime setup must resolve the active prompt, compact skill manifest, `readSkill(skillId)`, model config, and tool boundary before task start.
...
Akka autonomous `AgentDefinition` is the SDK builder result returned by `AutonomousAgent.definition()` or supplied via `AgentSetup`. It is **not** the skills pack's governed managed-agent `AgentDefinition` domain concept. Generated-app autonomous workers use both: the SDK definition for Akka task acceptance and the governed managed `AgentDefinition` for runtime behavior/profile resolution. When both appear, qualify them.
...
- governed managed `AgentDefinition`, active prompt, skill manifest, model config, provider secrets, and dynamic setup fail closed when invalid;
- `readSkill(skillId)` is registered for every managed autonomous worker and denies unassigned/empty-manifest loads safely with trace;
...
- worker progress/results emit typed `worker.task.*` workstream events, update attention, and render backend-projected surfaces when visible;
