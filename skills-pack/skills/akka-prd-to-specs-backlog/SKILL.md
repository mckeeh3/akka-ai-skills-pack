...
1. Requested readiness scope and fixed package `ai.first`.
2. Existing-app constraints and files to preserve.
3. AI-first signals: delegated work, retained human authority, goals/plans, policies, decisions, approvals, evidence, risk, traces, outcomes, supervision/governance UI.
4. Workstream split/merge decision and functional/context-area agents.
5. Attention categories and role-specific dashboard contracts.
6. Human surface graph: dashboard trunk, surface nodes, system messages, actions/events, result surfaces, trace links, realtime/refresh behavior, states.
7. Workstream expert bundles for LLM-backed functional agents: prompt intent, model binding, skills, references, manifests, tool boundaries, loaders, denials, traces, default-content governance, user-help examples, tests.
8. Governed capabilities and governed tools: ids/classes, actors/callers, AuthContext, tenant/customer scope, schemas, side effects, idempotency, policy/approval, audit/trace, exposure channels, tests.
9. Internal agent graph and AutonomousAgent task candidates when durable worker lifecycle fits.
10. Events, notifications, projections, and work traces that connect workstreams, dashboards, My Account, and left rail attention indicators.
11. Akka substrate/component mapping and implementation order.

If an item is unresolved and blocks safe planning, create/update `specs/pending-questions.md` instead of guessing.

## Full-core planning gate

Before creating specs/backlogs/pending tasks, classify readiness with `../docs/full-core-foundation-readiness.md`:

- `core app baseline`: five-core-workstream shell from `../docs/minimum-ai-first-saas-app.md`; not full-core-ready; queue explicit follow-up gates.
- `full core`: include canonical full-core foundation, invitation/onboarding, user/admin, governed runtime agents, workstream UI, audit/governance, and tests before app-specific slices.
- narrower scope: name it and list deferred full-core areas.

Do not create a full-core queue that collapses or omits core workstreams, Invitation onboarding, governed runtime agents, workstream UI, or required tests.

## Output file set

Create/update only the files justified by scope and project history.

### Required top-level files

- `specs/README.md`
- `specs/akka-solution-plan.md`
- `specs/pending-questions.md` when unresolved decisions exist
- `specs/pending-tasks.md` when implementation work is sufficiently unblocked

### Cross-cutting specs

For generated SaaS PRDs, create foundation specs first unless explicitly non-SaaS reference material:

- `specs/cross-cutting/00-common-domain-and-conventions.md` — fixed package `ai.first`, naming, shared contracts.
- `specs/cross-cutting/01-auth-tenancy-audit.md` — Account/Profile/Settings, Tenant/Customer, Membership/Role/Capability, WorkOS/JWT, `/api/me`, authorization, audit, support access, billing boundary, tenant-isolation tests.
- `specs/cross-cutting/02-ui-style-guide.md` when browser UI style is selected.
- Additional numbered cross-cutting specs only for real shared concerns.

### Module/sprint specs for larger PRDs

For large/multi-module/backend+frontend plans, prefer:

- `specs/modules/01-<module>.md`
- `specs/sprints/01-<vertical>-sprint.md`

Modules define durable boundaries. Sprints define ordered vertical full-stack increments. Use `../docs/module-sprint-planning.md`.

### Slice specs for smaller plans

For smaller or existing slice-based plans, prefer:

- `specs/slices/01-<slice>.md`
- `specs/slices/02-<slice>.md`

Do not create both `slices/` and `sprints/` for new output unless project history requires it.

### Build backlogs and task briefs

Create matching numbered backlog files:

- `specs/backlog/README.md`
- `specs/backlog/01-<sprint-or-slice>-build-backlog.md`

Create `specs/tasks/**` briefs only when a backlog item is still too large or ambiguous for one focused harness run.

## Required content by artifact

### Solution plan

Include: inputs, scope label, fixed package, existing-app constraints, AI-first interpretation, foundation obligations, workstream decision, surfaces, capabilities/governed tools, component mapping, skill routing, questions, implementation order, and tests.

### Sprint/slice spec

Include: purpose, scope, actors, workstreams/surfaces, capabilities, data/state, component candidates, API/UI contracts, authorization, side effects, traces, tests, dependencies, and done criteria.

### Backlog file

Include: implementation items in dependency order, each with capability/surface ids, files/classes likely touched, required reads, focused skills, acceptance criteria, tests, blockers, and size small enough for harness task selection.

### Pending questions

Use `../docs/pending-question-queue.md`. Ask one clear question per item. Record status, scope, why it blocks, options if known, affected specs/tasks, and reconciliation rule.

### Pending tasks

Use `../docs/pending-task-queue.md`. Each task should include id, status, source spec/backlog, brief path when present, required reads, focused skills, acceptance checks, blockers/questions, and notes for preserving auth/scope/audit/runtime completion.
