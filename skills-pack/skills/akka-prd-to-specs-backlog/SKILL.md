...
- `core app baseline`: for minimum/core app/basic/chatbot-like generated SaaS requests, plan the five core workstream core app domain from `docs/minimum-ai-first-saas-app.md`: My Account, User Admin, Agent Admin, Audit/Trace, and Governance/Policy functional agents; bootstrap-authorized user; selected AuthContext; durable workstream log; `markdown_response` system-message surfaces; backend capability boundaries; audit/work trace substrate; markdown sanitization; and core app tests. Every generated artifact must say it is not full core and must queue follow-up work for full User Admin, Agent Admin, Audit/Trace UI/search, invitation/onboarding, governed prompts/skills/references/manifests/tool boundaries, support access, billing boundary, full tenant-isolation/security tests, and app-specific workstreams.
