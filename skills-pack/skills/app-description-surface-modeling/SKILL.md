...
- For dashboards, model attention and next-action visuals as interactive work objects by default: cards, rows, counters, badges, chart segments, task/progress panels, shortcuts, icons, and buttons should declare the surface/action they open, the governed capability/browser-tool if protected, and the request/result surfaces they append. Use `none` with a recorded reason only for explicit non-actionable exceptions; ready dashboards should normally omit forbidden targets instead of displaying disabled work objects.
