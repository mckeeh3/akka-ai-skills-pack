...
Create only files justified by the app, but do not omit UI realization for generated AI-first SaaS. For SaaS Foundation App scope, managed-agent UI surfaces such as agent catalog/detail, prompt and skill governance, skill manifests and tool permissions, workstream tool-catalog/adapter visibility, and edit-agent proposals/traces belong to the foundation domain unless a task explicitly narrows or removes that area.
