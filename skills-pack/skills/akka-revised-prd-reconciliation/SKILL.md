...
- preserve or update core-baseline/full-core readiness sections when the revised PRD changes foundation scope; minimum/basic/core app/chatbot-like generated SaaS still starts from the five-core workstream domain shell, and any narrower User-Admin-only or page/chat-only task shape must be corrected, blocked, or explicitly marked non-SaaS/mechanics-only
