...
- mandatory agent workstream shell with role-authorized functional agents, universal workstream icon metadata for visible shell launchers/status panels, continuous main workstream, persistent composer, context/authority indicators, and structured surfaces for sign-in, context selection, profile/settings, Users, Invitations, Roles/Memberships, Access Review, Support Access, Admin Audit, Organization/Customer Settings, supervision, decisions, governance, audit/traces, and outcome review
...
- `app-description-functional-agent-modeling` — update the current-intent graph's global/domain worker and workstream nodes for role-authorized user-facing context-area agents, workstream icon metadata (`WorkstreamIconDescriptor` semantics: stable icon id, visual hint derived from workstream responsibility, accent color token, tooltip, aria label, optional asset reference; rendered through approved SVG/icon-library artwork or semantic SVG fallback, not letters), prompt intent, skills, tools, surfaces, capabilities, traces, and tests; maintained numbered files such as `12-workstreams/functional-agents.md` are legacy compatibility mappings, not the default shape for new work
- `app-description-surface-modeling` — update the current-intent graph's surface nodes and workstream surface contracts for structured workstream surfaces, typed payloads, reusable functional-agent placement, capability-backed actions including prompt/action/deep-link shell requests, states, traces, and tests; maintained numbered files such as `12-workstreams/surfaces-index.md` and `surface-contracts/**` are compatibility mappings when present
...
- `app-description-ui` — update the current-intent frontend/UI realization nodes after functional-agent/surface/capability meaning is clear, including a style contract for generated full-stack AI-first SaaS apps; maintained numbered files such as `55-ui/style-guide.md` are legacy compatibility mappings when present
...
This is the one-task queue-consumption skill for downstream implementation work. It selects one runnable task, prefers a fresh context session, loads only that task's required reads and skills, updates the task status, and reports the next runnable pending task.
