...
→ workforce decomposition: human workers, functional-agent workers, internal/autonomous/evaluator agent workers, and system workers
...
A workstream dashboard is a role-specific situational-awareness surface scoped primarily to one workstream and AuthContext. It is not a generic analytics dashboard; its primary objective is to show what requires this actor's attention and what work can or should be done next. Dashboard attention and next-action visuals are interactive work objects by default, not static summaries with a few separate buttons. If a card, row, counter, badge, chart segment, task/progress panel, icon, shortcut, or button represents a thing needing attention or a thing the actor can do, clicking or keyboard-activating it should append a request-like workstream item and append the detail, decision, progress, evidence/trace, result, updated dashboard, or `system_message` surface where the actor can inspect details and take allowed actions. A rectangular tile/card/counter with the thing's name and a large count is itself the button; count `0` is still valid when it opens an empty queue, detail, explanation, setup, or history surface. Non-actionable visuals are explicit exceptions and should be visually distinct with a recorded reason. Ready dashboards should show authorized work this actor can do; forbidden targets should normally be omitted rather than displayed as disabled dashboard objects. The same workstream may have different dashboard variants for Organization Admin, Customer Admin, Auditor, SaaS Owner support, or other authorized roles. It should answer:
...
- per-card/per-row/per-counter interaction contracts: full-shape hit area, zero-count behavior where applicable, `open_attention_item`, `show_surface`, `refresh_surface`, `open_trace`, governed browser-tool action, or `none` with reason;
...
> There are N items in this workstream that currently require this user's attention, given their identity, organization/customer context, memberships, roles, permissions, workstream availability, and authority.
