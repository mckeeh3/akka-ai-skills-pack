...
export class FixtureApiClient implements ApiClient {
  ...
  admin = {
    ...
    archiveOrganization: (organizationId: string, request: { reason: string; idempotencyKey: string }) => organizationLifecycle(this.organizations, organizationId, request, 'archived'),
    ...
    inviteSaasOwnerAdmin: (request: { email: string }) => delayedOk({ invitationId: `invite-saas-owner-${Date.now()}`, status: 'pending' as const, invitedEmail: request.email, correlationId: correlationId() }),
    resendSaasOwnerAdminInvitation: (invitationId: string) => delayedOk({ invitationId, status: 'resent' as const, invitedEmail: 'saas-owner@example.test', correlationId: correlationId() }),
    revokeSaasOwnerAdminInvitation: (invitationId: string) => delayedOk({ invitationId, status: 'resent' as const, invitedEmail: 'saas-owner@example.test', correlationId: correlationId() }),
    ...
    inviteOrganizationAdmin: (_organizationId: string, request: { email: string }) => delayedOk({ invitationId: `invite-org-admin-${Date.now()}`, status: 'pending' as const, invitedEmail: request.email, correlationId: correlationId() }),
    resendOrganizationAdminInvitation: (_organizationId: string, invitationId: string) => delayedOk({ invitationId, status: 'resent' as const, invitedEmail: 'org-admin@example.test', correlationId: correlationId() }),
    revokeOrganizationAdminInvitation: (_organizationId: string, invitationId: string) => delayedOk({ invitationId, status: 'resent' as const, invitedEmail: 'org-admin@example.test', correlationId: correlationId() }),
    changeOrganizationAdminRoles: (_organizationId: string, accountId: string, request: { roles?: string[]; idempotencyKey: string }) => delayedOk({ status: 'accepted', message: 'Organization Admin roles changed through backend authority.', membershipId: `membership-${accountId}`, accountId, roles: request.roles ?? ['TENANT_ADMIN'], membershipStatus: 'active', traceId: 'trace-org-admin-role-change', correlationId: correlationId() }),
    changeOrganizationAdminStatus: (_organizationId: string, accountId: string, status: 'suspend' | 'reactivate' | 'remove', _request: { idempotencyKey: string }) => delayedOk({ status: 'accepted', message: 'Organization Admin status changed through backend authority.', membershipId: `membership-${accountId}`, accountId, roles: ['TENANT_ADMIN'], membershipStatus: status === 'reactivate' ? 'active' : status === 'remove' ? 'removed' : 'suspended', traceId: 'trace-org-admin-status-change', correlationId: correlationId() }),
    ...
    archiveCustomer: (customerId: string, _request: { reason: string; idempotencyKey: string }) => delayedOk({ status: 'accepted', message: 'Customer archived through backend authority.', customer: customerDetail({ customerId, customerName: `Customer ${customerId}`, status: 'archived', traceRefs: ['trace-customer-archive'] }), traceRefs: ['trace-customer-archive'], correlationId: correlationId() }),
    ...
    inviteCustomerAdmin: (_customerId: string, request: { email: string }) => delayedOk({ invitationId: `invite-customer-admin-${Date.now()}`, status: 'pending' as const, invitedEmail: request.email, correlationId: correlationId() }),
    resendCustomerAdminInvitation: (_customerId: string, invitationId: string) => delayedOk({ invitationId, status: 'resent' as const, invitedEmail: 'customer-admin@example.test', correlationId: correlationId() }),
    revokeCustomerAdminInvitation: (_customerId: string, invitationId: string) => delayedOk({ invitationId, status: 'resent' as const, invitedEmail: 'customer-admin@example.test', correlationId: correlationId() }),
    changeCustomerAdminRoles: (_customerId: string, accountId: string, request: { roles?: string[]; idempotencyKey: string }) => delayedOk({ status: 'accepted', message: 'Customer Admin roles changed through backend authority.', membershipId: `membership-${accountId}`, accountId, roles: request.roles ?? ['CUSTOMER_ADMIN'], membershipStatus: 'active', traceId: 'trace-customer-admin-role-change', correlationId: correlationId() }),
    changeCustomerAdminStatus: (_customerId: string, accountId: string, status: 'suspend' | 'reactivate' | 'remove', _request: { idempotencyKey: string }) => delayedOk({ status: 'accepted', message: 'Customer Admin status changed through backend authority.', membershipId: `membership-${accountId}`, accountId, roles: ['CUSTOMER_ADMIN'], membershipStatus: status === 'reactivate' ? 'active' : status === 'remove' ? 'removed' : 'suspended', traceId: 'trace-customer-admin-status-change', correlationId: correlationId() })
  };
...
function organizationLifecycle(organizations: OrganizationSummary[], organizationId: string, request: { reason: string; idempotencyKey: string }, nextStatus: 'active' | 'suspended' | 'archived') {
