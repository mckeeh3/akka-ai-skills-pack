...
test('persistent composer is selected-agent aware and exposes disabled states', () => {
  ...
  assert.match(composer, /window\.addEventListener\('focus', restoreComposerFocus\)/);
  ...
  assert.match(composer, /pointerStartedInSelectableSurfaceRef/);
  assert.match(composer, /hasActiveTextSelection/);
  assert.match(composer, /isWorkstreamSelectableSurfaceTarget/);
  assert.match(composer, /document\.addEventListener\('pointerdown', recordSelectableSurfacePointer, true\)/);
  ...
  assert.match(componentsCss, /\.workstream-flow-entry,[\s\S]*?\.surface-frame \*[\s\S]*?user-select: text;/);
