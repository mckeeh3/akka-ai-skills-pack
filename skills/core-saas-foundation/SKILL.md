...
- Complete email-invite onboarding is mandatory: production readiness defaults to Resend (resend.com) for configured email delivery; use another provider only when the project explicitly records an accepted override decision; local/dev/test environments may use an explicit safe adapter that captures emails in an outbox without external delivery. Missing Resend configuration blocks readiness unless the project explicitly records an alternate provider override decision and equivalent configuration.
...
- Invitation email delivery uses Resend (resend.com) configuration for production readiness by default, or local/dev/test uses an explicit captured outbox adapter; delivery failures are visible to admins and auditable; alternate production providers require an accepted override decision; focused implementation routes through `akka-saas-invitation-onboarding` for InvitationWorkflow, email delivery/outbox Consumer, expiry/reminder TimedAction, InvitationView, admin endpoints/UI, and lifecycle tests.
