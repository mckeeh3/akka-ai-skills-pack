...
Use canonical doctrine plus this skill for surface selection. Also use `../../docs/agent-workstream-application-architecture.md` and `../../docs/structured-surface-contracts.md` when deciding where a surface belongs in the agent workstream model. Use `../../docs/examples/ai-first-dca-app-description/` as a worked AI-first UI reference when an example is useful, but do not treat consolidated historical UI files as canonical if they conflict with functional-agent workstream placement. Archived inbox UI notes are provenance only, not operative guidance.
