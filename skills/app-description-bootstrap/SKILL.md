...
These must seed the mandatory secure SaaS foundation from `core-saas-foundation`; WorkOS is the default browser authentication provider and Resend (resend.com) is the default production invite-email provider. Provider-specific setup values may remain explicit open questions, and non-default providers require an accepted override decision, but authorization, tenancy, audit, and tenant isolation are not optional. The bootstrap must state that no route, agent tool, data access, workflow action, view query, stream, or generated UI action is public or authorized by default except deliberately public static assets.
