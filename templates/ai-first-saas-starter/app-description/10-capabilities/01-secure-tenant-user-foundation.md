...
This is a reference capability contract for the skills pack's scaffolded core app description, not this repository's own business application. It demonstrates the expected app-description capability shape for generated secure AI-first SaaS apps.
...
- Complete invitation lifecycle: create, send, resend, revoke, expire, accept, delivery status, delivery attempts, expired-invitation dashboard attention, safe resend/reinvite decisions, and audit.
...
| `admin.invitations.resend` / `admin.invitations.revoke` | command/workflow | all three User Admin surfaces; dashboard expired-invitation attention | Tenant Admin, Customer Admin, support operator only when grant includes invitation support, approved workflow; `InvitationDraftAgent` may prepare rationale only | Require scoped invitation id and no raw token exposure. Resend is idempotent and audited; expired invitations either route to reinvite/resend policy or deny with safe explanation. Deny cross-scope invitation, missing invitation capability, disabled actor, and SaaS Owner no support access. |
