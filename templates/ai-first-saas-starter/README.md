...
- remaining slices: event-sourced invitation lifecycle history, richer event-sourced PromptDocument/SkillDocument/ReferenceDocument/manifest/ToolPermissionBoundary lifecycle history, InvitationView/UserDirectoryView/AdminAuditView/governed-agent projections, extended foundation projections beyond the starter's current Akka-backed scope, broader event coverage beyond the bounded v3 starter event families, enterprise notification/digest infrastructure, broader task/AutonomousAgent notification coverage beyond the implemented access-review, prompt-risk, and governance-impact queued/running/blocked/failed/completed-review/cancelled/accepted/rejected starter states, and a future policy simulation platform beyond the bounded Governance/Policy impact-analysis worker.
