...
As of the `specs/full-core-smb-runtime-durability-remediation/` remediation pass on 2026-05-30, the prior `specs/full-core-smb-polish-release-readiness/` ship recommendation remains **blocked for the stronger no-in-memory-normal-runtime bar** until all queued backend, frontend, static-asset, and release-handoff tasks pass. Backend foundation workstream-log, invitation, governed-agent behavior, and agent runtime trace normal paths now bind Akka where `ComponentClient` is available, while identity, access-review, audit, and governance ports fail closed unless the scaffold is explicitly opted into local/demo repositories with `AI_FIRST_SAAS_LOCAL_DEMO_REPOSITORIES=true`. Frontend/static fixture gating and final release validation remain queued before claiming completed normal generated runtime has no in-memory/default fixture dependencies.
