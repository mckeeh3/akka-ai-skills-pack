...
- test-only adapters live under backend test source; normal runtime paths use Akka component-backed repositories/sinks once the Akka `ComponentClient` binding is available, and missing bindings fail closed with actionable guidance instead of selecting substitute state;
- durable normal path present for workstream messages where `ComponentClient` is available: `WorkstreamEndpoint` binds `AkkaWorkstreamLogRepository`, Akka invitation and governed-agent repositories, and durable agent runtime trace storage; service construction without `ComponentClient` is an unbound pre-runtime state, not a completed feature path;
- remaining slices: event-sourced invitation lifecycle history, richer event-sourced PromptDocument/SkillDocument/ReferenceDocument/manifest/ToolPermissionBoundary lifecycle history, InvitationView/UserDirectoryView/AdminAuditView/governed-agent projections, and extended foundation projections beyond the starter's current Akka-backed scope.
...
As of the real Akka runtime replacement work on 2026-05-30, the starter is **release-ready for the stronger no-in-memory-normal-runtime bar at the documented SMB starter scope**. Fullstack rendered validation passed after backend, frontend, and static-asset remediation. Normal completed runtime paths no longer silently depend on in-memory/default fixture stores: workstream-log, identity, invitation, access-review, audit, governance, governed-agent behavior, and agent runtime trace paths bind Akka durable components when `ComponentClient` is available; non-ComponentClient foundation ports fail closed with actionable guidance and do not expose substitute normal runtime state; frontend fixture mode requires explicit dev/local opt-in and production-like static resources scan clean for fixture/demo/provider-secret markers.
