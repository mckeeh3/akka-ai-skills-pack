...
- local/demo adapters retained for tests and explicit local inspection: set `AI_FIRST_SAAS_LOCAL_DEMO_REPOSITORIES=true` only for local/demo use; without that opt-in, foundation identity, access-review, audit, governance, and non-ComponentClient workstream log/runtime paths fail closed with actionable repository-binding guidance;
- durable normal path present for workstream messages where `ComponentClient` is available: `WorkstreamEndpoint` binds `AkkaWorkstreamLogRepository`, Akka invitation and governed-agent repositories, and durable agent runtime trace storage; service construction without `ComponentClient` uses fail-closed ports instead of silent volatile defaults;
- remaining slices: durable Account/Profile/Settings/Membership, Tenant/Customer, AdminAuditEvent history, durable access-review task lifecycle, durable Governance/Policy proposal lifecycle, event-sourced invitation lifecycle history, richer event-sourced PromptDocument/SkillDocument/ReferenceDocument/manifest/ToolPermissionBoundary lifecycle history, InvitationView/UserDirectoryView/AdminAuditView/governed-agent projections, and endpoint-level binding for foundation identity/audit/governance repositories.
...
As of the `specs/full-core-smb-runtime-durability-remediation/` validation pass on 2026-05-30, the starter is **release-ready for the stronger no-in-memory-normal-runtime bar at the documented SMB starter scope**. Fullstack rendered validation passed after backend, frontend, and static-asset remediation. Normal completed runtime paths no longer silently depend on in-memory/default fixture stores: workstream-log, invitation, governed-agent behavior, and agent runtime trace paths bind Akka durable components when `ComponentClient` is available; identity, access-review, audit, governance, and non-ComponentClient foundation ports fail closed with actionable guidance unless the scaffold is explicitly opted into local/demo repositories with `AI_FIRST_SAAS_LOCAL_DEMO_REPOSITORIES=true`; frontend fixture mode requires explicit dev/local opt-in and production-like static resources scan clean for fixture/demo/provider-secret markers.
