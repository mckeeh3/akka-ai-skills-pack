...
- SaaS Owner Admins may invite and manage other SaaS Owner Admin users only from a SaaS Owner selected `AuthContext` with backend `saas_owner.user.manage`, with last-owner-admin and privilege-escalation protections.
...
| `tenant.customer.list` | protected read | List/search browser-safe Customers within the selected Organization/Tenant visible to an Organization Admin. | `surface-user-admin-customer-directory`; `GET /api/admin/customers`. |
| `tenant.customer.read` | protected read | Read one Customer detail and safe lifecycle/admin metadata within the selected Organization/Tenant without exposing sibling customers or hidden counts. | `surface-user-admin-customer-detail`; `GET /api/admin/customers/{customerId}`. |
| `tenant.customer.create` | command | Create a Customer record under the selected Organization/Tenant with normalized name, idempotency key, validation, and audit/work trace evidence. | `surface-user-admin-customer-create`; `POST /api/admin/customers`; success opens detail. |
| `tenant.customer.rename` | command | Rename/update a Customer display profile with validation, no-op replay semantics, idempotency, and audit/work trace evidence. | `surface-user-admin-customer-rename`; `POST /api/admin/customers/{customerId}/rename`; success refreshes detail. |
| `tenant.customer.suspend` | command | Suspend/archive a Customer lifecycle boundary without exposing or mutating unrelated customer application data; records reason, idempotency, warnings, and audit/work trace evidence. | `surface-user-admin-customer-suspend-confirmation`; `POST /api/admin/customers/{customerId}/suspend`; success refreshes detail. |
| `tenant.customer.reactivate` | command | Reactivate a suspended Customer with no-op handling, idempotency key, and audit/work trace evidence. | `surface-user-admin-customer-reactivate-confirmation`; `POST /api/admin/customers/{customerId}/reactivate`; success refreshes detail. |
...
In scope: governed user/admin operations, SaaS Owner Admin user management, SaaS Owner Organization administration, SaaS Owner bootstrap/maintenance of Organization Admin users, Organization Admin Customer administration, Organization Admin bootstrap/maintenance of Customer Admin users, invitation lifecycle, member status/role changes, support-access lifecycle, access-review recommendations, decision cards for risky changes, admin audit evidence, scoped User Admin agent guidance, and safe local/test email outbox behavior.
