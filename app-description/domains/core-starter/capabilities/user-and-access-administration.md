...
| `user_admin.acceptance_status.read` | protected read | Invitation expiry, accepted, failed-delivery, bounced, pending, and recovery evidence shaping for administrators and safe invitee onboarding recovery. | Dashboard, `surface-user-admin-invitation-detail`, directory badges, invitation acceptance recovery. |
| `user_admin.accept_invitation` | onboarding command | Validate signed invitation token, WorkOS/AuthKit-authenticated invitee identity, email/account eligibility, invitation status, expiry, revocation, target app-owner/tenant/customer scope, requested role, duplicate membership, and stale token state; create or link the account/membership and mark the invitation accepted idempotently. | Invitation acceptance/onboarding endpoint and safe onboarding result/system-message; success establishes or refreshes the accepted user's eligible selected `AuthContext`. |
...
- `create-or-resend-invitation` (`browser-tool`; agent may prepare human-confirmed payloads): admin invitation lifecycle, Resend/captured-outbox delivery boundary, and browser-safe acceptance status reads.
- `accept-invitation` (`onboarding-tool`; invitee/system only): standard invitee acceptance through signed invitation token plus WorkOS/AuthKit authenticated identity; not exposed to agents for authority expansion and not usable for admin-side role changes.
...
Every admin command/query is scoped by selected `AuthContext`, active account, non-disabled actor status, tenant/customer ids, resource ownership, membership status, role/capability grants, support-access state, and approval policy. Invitation acceptance is scoped by signed invitation token, WorkOS/AuthKit-authenticated invitee identity, invitation target scope, target role, and backend lifecycle validation; it does not trust browser-provided tenant/customer/role input as authority. Cross-tenant/customer access, disabled users, inactive memberships, email-only authorization without a valid invitation token and authenticated identity, prompt-only privilege grants, hidden-target enumeration, raw token/secret access, unredacted audit export, unsupported bulk side effects, last-admin loss, self-disable/self-admin-role-removal, role escalation, tenant/customer application-data access through Organization Admin, support access through Organization Admin, billing-derived authority, and provider/model/outbox fake success are forbidden.
