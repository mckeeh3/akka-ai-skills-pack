...
| `user-admin` | `ready-to-build` | Three admin levels, user/org/customer/invitation/membership/role/support-access/access-review/identity-exception behavior, capability bindings, surface graph, tools, tests, traces, and realization mappings are present. | Local Akka/API/UI validation of scoped authorization, idempotent admin mutations, invitation delivery/acceptance boundaries, last-admin/self-action guardrails, audit/work traces, and provider/outbox/model fail-closed behavior. |
| `agent-admin` | `ready-to-build` | SaaS-admin-only managed-agent document editing intent, functional agent, access, behavior, versioning lifecycle, tools, surfaces, tests, traces, and realization mappings are present. | Local Akka/API/UI validation of SaaS-admin authorization, catalog/detail browsing, AI-assisted edit draft/review/save/cancel/restore, version history, runtime doc loading, and trace visibility. |
...
| `audit-trace` | `ready-to-build` | Search/inspect/explain/redact/summarize/export/annotate intent, functional agent, attention categories, access, behavior, surfaces, tools, tests, traces, and realization mappings are present. | Local Akka/API/UI validation of scoped trace search/detail/timeline/failure/export/note flows, redaction, authorization denials, provider/model fail-closed summary behavior, and durable audit/work traces. |
