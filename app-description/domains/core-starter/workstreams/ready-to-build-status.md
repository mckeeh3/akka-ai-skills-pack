...
| `audit-trace` | `ready-to-build` | V1 tenant-admin activity-log intent, access, behavior, surfaces, tools, tests, traces, retention rules, and realization mappings are present for search, full-payload detail, tool-call linkage, denial evidence, and configurable retention. | Local Akka/API/UI validation of tenant-admin scoped search, detail/full-payload warning, tool-call parent links, retention default/update/bounds/no-op behavior, non-admin/disabled/cross-tenant denials, immutable trace records, and durable audit/work traces. |
