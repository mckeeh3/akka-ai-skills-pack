...
The workstream owns `audit-trace-agent` as its exactly-one user-facing functional-agent binding for workstream navigation and explanation copy only. In the tenant-admin activity-log scope, the agent does not receive trace-search or trace-detail governed-tool authority and cannot reveal audit payloads through chat. Tenant-admin trace access is through protected browser surface actions backed by backend authorization.
