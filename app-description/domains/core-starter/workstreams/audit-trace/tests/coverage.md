...
- Surface/action coverage includes dashboard refresh, search, detail, timeline, failure evidence, investigation guide, redacted export request, investigation note append, summary task start/read, summary review open, summary accept, and summary reject.
...
- Repeated export requests, investigation note appends, summary starts, summary reads, and summary accept/reject decisions do not duplicate effects and retain correlation/trace evidence.
- Denials, approval-required outcomes, provider fail-closed states, tool-boundary denials, retention/redaction outcomes, stale/conflict results, hidden/not-found results, and trace emissions are verifiable through local Akka/API/UI tests or readiness evidence.
