...
| `route-audit-trace-activity-log-open-v1` | Audit/Trace | `open audit trace`, `show audit activity log`, `search traces` | `surface-audit-trace-activity-log` | `audit.trace.search` | none | Open-only tenant-admin activity log; no export, note append, AI summary, or hidden trace enumeration. |
| `route-audit-trace-retention-settings-open-v1` | Audit/Trace | `audit retention settings`, `change audit retention` | `surface-audit-trace-retention-settings` | `audit.trace.retention.read` | none | Open-only retention settings; saving requires explicit form submit and backend validation. |
...
| Audit/Trace | none in the tenant-admin activity-log scope | none | none | none | none; tenant-admin activity-log scope Audit/Trace does not expose `human_chat_tool_plan` execution. |
...
| Audit/Trace | `router-only` for opening activity log and retention settings; browser surface actions handle search/detail/retention after backend authorization. | The tenant-admin activity-log scope has no chat-executable Audit/Trace entries. Export, notes, AI summaries, timeline/failure/guide surfaces, raw evidence chat delivery, hidden trace delivery, and ingestion/projection internals are out of scope or blocked pending later design. |
...
| Surface id/title | Purpose | Prompt examples | Required capabilities | Prefill fields | Forbidden direct effects |
|---|---|---|---|---|---|
| `surface-audit-trace-activity-log` — Audit Activity Log | Tenant-admin searchable activity log for human request/response, agent request/response, tool calls, denials, failures, and retention-setting changes. | `open audit trace`, `show audit activity log`, `search traces`, `who did what?` | `audit.trace.search` | none in deterministic routing; filters are edited on the surface | Do not search full payloads, reveal hidden traces, export evidence, append notes, generate summaries, or grant agent/tool authority. |
| `surface-audit-trace-detail` — Trace Detail | Authorized trace detail with full payloads and sensitive tenant-admin warning. | `open trace detail`, `show this audit event` | `audit.trace.detail.read` | visible trace handle only from authorized rows/links | Do not edit evidence, reveal cross-tenant records, show hidden trace existence, or expose secrets/tokens/provider credentials. |
| `surface-audit-trace-retention-settings` — Audit Retention Settings | Tenant-admin retention setting with default 90 days and allowed 30–365 day range. | `audit retention settings`, `change audit retention` | `audit.trace.retention.read`; `audit.trace.retention.update` only on explicit submit | none in deterministic routing | Do not save from router/chat, accept values outside 30–365 days, or mutate historical trace records. |
| `surface-audit-trace-system-message` — Audit Trace System Message | Safe validation, forbidden, not-found/redacted, no-op, saved, or failure feedback. | `why was audit trace denied?` | safe selected-context Audit/Trace visibility | none | Do not enumerate hidden targets, leak policy internals, or convert denial into authority. |

Deferred or ambiguous Audit/Trace routing: tenant-admin activity-log scope deterministic routing may open the activity log or retention settings only. Trace detail requires a backend-visible authorized row/link. Export, investigation notes, suspicious-activity acknowledgement, AI summaries, timeline/failure/guide surfaces, full-payload keyword search, and chat/tool execution are out of scope for the tenant-admin activity-log scope.
