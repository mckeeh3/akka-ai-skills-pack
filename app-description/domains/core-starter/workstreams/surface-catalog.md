...
| Audit/Trace | none in v1 | none | none | none | none; v1 Audit/Trace does not expose `human_chat_tool_plan` execution. |
...
| Audit/Trace | `router-only` for opening activity log and retention settings; browser surface actions handle search/detail/retention after backend authorization. | V1 has no chat-executable Audit/Trace entries. Export, notes, AI summaries, timeline/failure/guide surfaces, raw evidence chat delivery, hidden trace delivery, and ingestion/projection internals are out of scope or blocked pending later design. |
...
Deferred or ambiguous Audit/Trace routing: v1 deterministic routing may open the activity log or retention settings only. Trace detail requires a backend-visible authorized row/link. Export, investigation notes, suspicious-activity acknowledgement, AI summaries, timeline/failure/guide surfaces, full-payload keyword search, and chat/tool execution are out of scope for v1.
