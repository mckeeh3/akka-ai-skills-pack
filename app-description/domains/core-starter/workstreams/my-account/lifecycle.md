...
Last manual runtime test: 2026-06-30 — `RV-MY-ACCOUNT-001` was retried and blocked before authenticated API/UI execution by an over-strict runtime-validation preflight that treated placeholder `WORKOS_JWT_AUDIENCE` as fatal even though the local app can run without that setting; see `specs/runtime-validation/runs/2026-06-30-RV-MY-ACCOUNT-001-blocked-auth-config.md`. The preflight has been corrected and the task is pending rerun for actual startup/auth/API evidence.
...
- `RV-MY-ACCOUNT-001` is ready to rerun with the corrected runtime-validation preflight: `tools/runtime-validation/start-local.sh --empty` and `tools/runtime-validation/seed.sh base-organization` now provide the preferred start/seed contracts and member/disabled fixtures. Valid member login and `/api/me` evidence still require working local WorkOS/AuthKit issuer, public client id, redirect URI, and test-user mapping; `WORKOS_JWT_AUDIENCE` is no longer treated as a startup prerequisite.
