...
Last manual runtime test: 2026-06-30 — `RV-MY-ACCOUNT-001` was retried and blocked before authenticated API/UI execution by `auth-setup-blocker` (`WORKOS_JWT_AUDIENCE` placeholder), with `runtime-validation-gap` and `seed-data-blocker` consequences because the local Akka runtime did not start; see `specs/runtime-validation/runs/2026-06-30-RV-MY-ACCOUNT-001-blocked-auth-config.md`.
