...
Last manual runtime test: 2026-06-29 — `RV-MY-ACCOUNT-001` was attempted again and blocked before authenticated API/UI execution by `auth-setup-blocker` (`WORKOS_JWT_AUDIENCE` placeholder), with `runtime-validation-gap` and `seed-data-blocker` consequences because the local Akka runtime did not start; see `specs/runtime-validation/runs/2026-06-29-RV-MY-ACCOUNT-001-blocked-auth-config.md`.
...
- `RV-MY-ACCOUNT-001` is ready to rerun once real local WorkOS/AuthKit values are available: `tools/runtime-validation/start-local.sh --empty` and `tools/runtime-validation/seed.sh base-organization` now provide the preferred start/seed contracts and member/disabled fixtures, but valid member login and `/api/me` evidence still require non-placeholder WorkOS issuer/audience/client configuration at runtime. The latest blocked run specifically found `WORKOS_JWT_AUDIENCE` placeholder in the effective runtime environment.
