...
Minimum fields: trace/work trace id, correlation id, causation/parent event id, selected `AuthContext`, tenant/customer scope where applicable, functional agent `my-account-agent`, requestedBy, confirmedBy for execution, action ids `action-update-my-settings`, governed tool ids `my_account.update_profile_settings`, capability ids `my_account.update_profile_settings`, input schema ref, plan id, plan snapshot id, step id/sequence/dependencies, idempotency key or redacted hash, authorization decision and basis summary, policy/approval refs, prompt/skill/reference/model/tool-boundary refs for proposal generation, result surface ids `surface-my-settings`, status, safe error code, redaction classification, and browser-safe input/output summaries.
