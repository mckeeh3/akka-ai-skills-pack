...
2. **Prioritize by workstream** — render a top attention counter strip before profile/settings panels, using the web UI catalog `Attention counter card` anatomy. There must be one counter card for each currently available/authorized workstream. Each card shows the workstream label, attention count, status text, short purpose/context, severity, and an Open action to the workstream's default surface. This visual intentionally mirrors the left rail but provides more context.
3. **Avoid redundant detail clutter** — do not render a separate raw `Needs me` detail list on the dashboard when the per-workstream attention counter cards are present. Raw capability ids, governed tool ids, target surface ids, and trace ids must not appear as primary dashboard text. Detailed attention/evidence belongs in the opened source workstream or collapsed trace/evidence affordances.
4. **Control** — expose compact secondary panels for Profile, Settings, Context & authority, Notifications, and Personal digest/export. Future My Account-specific surfaces may be added here without changing the per-workstream attention-counter model.
5. **Explain** — every attention item shows why it is visible: source workstream, required capability, redaction level, trace/correlation links, and safe target hints.
...
- `attentionCounters[]`: one entry per available authorized workstream with stable counter id, workstream id, label, attention count value, severity, status text, short workstream purpose/context, source capability/tool, target surface id, and open action id.
- `needsAttention[]`: backend payload may retain detailed attention item metadata for traceability/reuse, but the dashboard does not render it as a raw list when `attentionCounters[]` are present.
