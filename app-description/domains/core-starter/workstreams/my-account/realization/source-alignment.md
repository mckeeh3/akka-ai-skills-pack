...
| `my-account.trace-audit` | Durable work/audit traces and browser-safe trace refs | `blocked-validation` | Targeted backend trace evidence exists for durable admin-audit/workstream-log trace checks covering My Account dashboard/profile/settings/context surface actions, profile save, notification center lifecycle/preference, personal attention digest fail-closed start, and `human_chat_tool_plan` proposal/confirmation/step execution trace facts; aggregate required Maven check is blocked by existing `AuditTraceBrowserWorkstreamSmokeTest` disabled-account denial failures. |
...
- Last aligned evidence: MAFA-03-001 adds `MyAccountTraceAuditTest`, proving durable browser-safe `admin-audit-event` facts for My Account surface actions with actor/account, selected `AuthContext`, tenant/customer scope, adapter/source, action/browser/governed-tool/capability ids, result state, idempotency hash, correlation id, result surface, redaction, and trace refs. The same test proves personal attention digest fail-closed start audit evidence, notification projection/lifecycle/preference audit evidence, durable `human_chat_tool_plan` proposed/confirmed/step-started/step-completed facts, confirmed chat action adapter classification, persisted workstream-log result entries, and Audit/Trace timeline retrieval of the chat-plan workstream log evidence. Targeted `mvn -Dtest='MyAccountTraceAuditTest' test` passes.
- Remaining validation gaps / blocker: the required aggregate Maven command still fails in `AuditTraceBrowserWorkstreamSmokeTest` because disabled Audit/Trace actors unexpectedly resolve instead of producing denial failures; MAFA-03-001 is left blocked until that existing Audit/Trace browser-smoke denial issue is repaired. Provider-backed digest success remains a provider-configuration dependency tracked by `my-account.digest-export`; broader frontend/manual trace-link rendering and later notification/chat-plan task-specific runtime depth remain unclaimed by this backend trace verification.
