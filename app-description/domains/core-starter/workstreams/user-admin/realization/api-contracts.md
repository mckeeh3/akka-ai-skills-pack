...
| `user_admin.list_members` / `user_admin.read_user_account` | `browser-tool`, `agent-tool` read | `AdminEndpoint.java`, `UserAdminService.java`, `UserDirectoryView.java`; identity repositories/views | Scoped search/list/detail with backend filters, pagination, no hidden counts, directory row/card selection into user detail, detail task entry points, and safe denials. |
