...
| `tenant.customer.*` / `manage-customers` | `browser-tool`; agent prepares human-confirmed payloads only | Admin API/workstream actions plus customer repository/service scoped to selected Organization/Tenant | Customer directory/detail/create/rename/suspend/reactivate surfaces; selected Tenant ownership, idempotency, no sibling-customer leakage, safe denials, and audit events. |
| `tenant.customer_admin.*` / `manage-customer-admins` | `browser-tool`; agent prepares human-confirmed payloads only | Admin API/workstream actions plus invitation/user-admin services scoped to selected Customer | Customer Admin list/invite/detail/manage surfaces; target Customer exists, `CUSTOMER_ADMIN` role validation, no `TENANT_ADMIN`/`SAAS_OWNER_ADMIN` escalation, last-customer-admin guardrails, idempotency, invitation/outbox delivery, safe denials, and audit events. |
