...
| Customer workstream action routing | `src/main/java/ai/first/application/coreapp/workstream/WorkstreamService.java` | Maps canonical Customer action ids `action-user-admin-show-customers`, `action-customer-read`, `action-open-customer-create`, `action-customer-create`, `action-open-customer-rename`, `action-customer-rename`, `action-open-customer-suspend`, `action-customer-suspend`, `action-open-customer-reactivate`, `action-customer-reactivate`, `action-user-admin-show-customer-admins`, `action-open-customer-admin-invitation-create`, and `action-customer-admin-invite` to Customer/Customer Admin surfaces with selected Tenant/Customer scope, idempotency, safe denials, no-op/conflict handling, redaction, and audit/work traces. Generic `action-customer-list`, `action-customer-admin-list`, and `action-customer-admin-manage` names are documentation-only shorthand unless explicitly implemented as aliases later. |
