...
| Organization/Tenant boundary lifecycle and Organization Admin branch | `src/main/java/ai/first/application/coreapp/useradmin/SaasOwnerOrganizationAdminService.java`, identity repository tenant methods under `src/main/java/ai/first/application/foundation/identity/**`, `src/main/java/ai/first/api/coreapp/admin/AdminEndpoint.java` | Realizes `saas_owner.organization.*` and `saas_owner.organization_admin.*` through backend-authorized Organization directory/detail/create/rename/suspend/archive/reactivate and Organization Admin invitation/membership operations. Archive is terminal normal-administration closure, not hard delete, and does not grant tenant app-data/support/billing authority. |
| Customer boundary lifecycle and Customer Admin branch | `src/main/java/ai/first/domain/foundation/identity/Customer.java`, `src/main/java/ai/first/application/coreapp/useradmin/TenantCustomerAdminService.java`, identity repository customer methods under `src/main/java/ai/first/application/foundation/identity/**`, `src/main/java/ai/first/api/coreapp/admin/AdminEndpoint.java` | Realizes `tenant.customer.*` and `tenant.customer_admin.*` through backend-authorized Customer directory/detail/create/rename/suspend/archive/reactivate and Customer Admin invitation/membership operations. The implementation boundary is foundation Customer state for authorization/audit, not CRM/customer-success/sales/billing/support business state; archive is terminal normal-administration closure, not hard delete. |
| Customer workstream action routing | `src/main/java/ai/first/application/coreapp/workstream/WorkstreamService.java` | Maps current product action ids `action-user-admin-show-customers`, `action-customer-read`, `action-open-customer-create`, `action-submit-customer-create`, `action-open-customer-rename`, `action-submit-customer-rename`, `action-open-customer-suspend`, `action-customer-suspend`, `action-open-customer-archive`, `action-customer-archive`, `action-open-customer-reactivate`, `action-customer-reactivate`, `action-user-admin-show-customer-admins`, `action-open-customer-admin-invitation-create`, and `action-customer-admin-invite` to Customer/Customer Admin surfaces with selected Tenant/Customer scope, idempotency, terminal archive no-reactivate behavior, safe denials, no-op/conflict handling, redaction, and audit/work traces. Retired compatibility names such as `action-customer-create`, `action-customer-rename`, `action-open-customer-suspend-confirmation`, `action-open-customer-reactivate-confirmation`, `action-customer-list`, `action-customer-admin-list`, and `action-customer-admin-manage` are not product action ids and must return safe unsupported-action/system-message results if submitted by stale clients. |
...
Implementation should map backend outcomes to `surface-user-admin-dashboard`, Organization branch surfaces (`surface-user-admin-organization-directory`, `surface-user-admin-organization-detail`, `surface-user-admin-organization-create`, `surface-user-admin-organization-rename`, `surface-user-admin-organization-suspend-confirmation`, `surface-user-admin-organization-reactivate-confirmation`, `surface-user-admin-organization-admins`, `surface-user-admin-organization-admin-invitation-create`, `surface-user-admin-organization-admin-detail`), Customer branch surfaces (`surface-user-admin-customer-directory`, `surface-user-admin-customer-detail`, `surface-user-admin-customer-create`, `surface-user-admin-customer-rename`, `surface-user-admin-customer-suspend-confirmation`, `surface-user-admin-customer-reactivate-confirmation`, `surface-user-admin-customer-admins`, `surface-user-admin-customer-admin-invitation-create`, `surface-user-admin-customer-admin-detail`), `surface-user-admin-users`, `surface-user-admin-invitation-detail`, `surface-user-admin-user-detail`, `surface-user-admin-role-change-preview`, `surface-user-admin-access-review-task`, reusable decision/audit/workflow/markdown surfaces, or `surface-user-admin-system-message`. Organization and Customer suspend-confirmation surfaces carry `lifecycleMode=suspend|archive`; archive outcomes are terminal no-reactivate results, not hard delete. Customer branch outcomes must preserve Organization/Tenant scope for Customer lifecycle and selected Customer scope for Customer Admin operations.
