...
| Customer workstream action routing | `src/main/java/ai/first/application/coreapp/workstream/WorkstreamService.java` | Maps current product action ids `action-user-admin-show-customers`, `action-customer-read`, `action-open-customer-create`, `action-submit-customer-create`, `action-open-customer-rename`, `action-submit-customer-rename`, `action-open-customer-suspend`, `action-customer-suspend`, `action-open-customer-reactivate`, `action-customer-reactivate`, `action-user-admin-show-customer-admins`, `action-open-customer-admin-invitation-create`, and `action-customer-admin-invite` to Customer/Customer Admin surfaces with selected Tenant/Customer scope, idempotency, safe denials, no-op/conflict handling, redaction, and audit/work traces. Runtime may accept compatibility aliases documented in the surface contracts such as `action-customer-create`, `action-customer-rename`, `action-open-customer-suspend-confirmation`, and `action-open-customer-reactivate-confirmation`; generic `action-customer-list`, `action-customer-admin-list`, and `action-customer-admin-manage` names remain documentation-only shorthand unless explicitly implemented as aliases later. |
