...
- Given an authorized caller with selected `AuthContext`, when they open User Admin, then the dashboard and allowed surfaces render only scoped data and expose only authorized actions.
- Given an allowed action, when it is submitted with valid input, then capability `user-and-access-administration` returns the expected structured result and emits required traces.
