...
- Given dashboard queue filters, when a user opens a queue, then `surface-user-admin-member-directory`, `surface-user-admin-invitation-panel`, or `surface-user-admin-access-review-task` renders with backend-shaped filters and no cross-scope leakage.
...
- Dashboard, member directory, invitation panel, user account detail, role-change preview, access-review task, decision-card, markdown-response, audit-link, and system-message variants render loading, empty, ready, submitting, validation-error, forbidden, not_found_or_redacted, conflict, stale/reconnect, partial-data, provider/model/outbox fail-closed, no-op, approval-required, and failure states.
