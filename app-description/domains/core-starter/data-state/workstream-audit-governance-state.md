...
The core starter retention default is conservative: security-relevant audit/work traces, admin lifecycle events, invitation acceptance evidence, provider-fail-closed evidence, policy/agent behavior decisions, and trace-gap records are retained for the lifetime of the local foundation app unless a later accepted retention policy adds purge/anonymization behavior. Browser export links and generated export bundles are short-lived and expire according to backend policy; expiry removes download availability, not source evidence. Legal hold, account erasure, provider-side deletion reconciliation, and bulk purge are out of scope until explicitly described with authorization, redaction, audit, tests, and provider-boundary contracts.
