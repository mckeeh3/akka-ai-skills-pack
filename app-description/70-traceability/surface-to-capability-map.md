...
| `audit-trace-explorer` / `surface.audit_trace.search.v1` / `surface.audit_trace.timeline.v1` / `surface.audit_trace.detail.v1` / `surface.audit_trace.investigation_note.v1` | Audit/Trace, User Admin, Agent Admin, Governance/Policy | search/inspect/timeline/append investigation note/export-if-enabled scoped traces → `governance-decisions-audit` (`audit.trace.search`, `audit.trace.detail`, `audit.trace.timeline`, `audit.trace.investigation_note.append`); prompt assembly, skill/reference load, tool-boundary, work and denial trace details → `managed-agent-foundation`. |
